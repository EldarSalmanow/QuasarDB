...
public:
    TablePage(uint32_t id, Pager* pager, void* mem_ptr = nullptr)
        : _pager(pager),
          _data(mem_ptr ? static_cast<DataPage*>(mem_ptr) : new DataPage(),
                PageDeleter{ mem_ptr == nullptr }
          )
    {
    ...
    Slot* get_slots() const { 
        return reinterpret_cast<Slot*>(_data->raw + sizeof(PageHeader)); 
    }

    uint32_t slot_count() const {
        return _data->header.slot_count;
    }

    uint32_t free_space_upper() const {
        return _data->header.free_space_upper;
    }
    ...
    uint32_t get_full_free_space_size() const {
        return _data->header.total_free_bytes;
    }
