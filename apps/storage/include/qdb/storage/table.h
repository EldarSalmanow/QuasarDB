...
class Table final {
    ...
    struct FastStr {
        ...
        bool operator==(const FastStr& other) const {
            return _record_id == other._record_id &&
                   std::memcmp(_prefix, other._prefix, PREFIX_SIZE) == 0;
        }

        int SearchCmp(FastStr other) const {
            return std::memcmp(_prefix, other._prefix, PREFIX_SIZE);
        }
    ...
    std::unordered_map<std::string, std::variant<
        BStarPlusTree<int32_t, RecordAddress>,
        BStarPlusTree<FastStr, RecordAddress>
    >> _indexes;
        ...
        for (size_t i = 0; i < _schema.size(); ++i) {
            ...
            if (column.not_null() && value.is_null()) {
                throw std::runtime_error("Column '" + column.name() + 
                                         "' cannot be NULL (NOT_NULL constraint)");
            }
            if (!value.is_null()) {
                if (column.is_int() && !value.is_int()) {
                    throw std::runtime_error("Column '" + column.name() + 
                                             "' expects INT, got " + value.type_name());
                }
                if (column.is_string() && !value.is_string()) {
                    throw std::runtime_error("Column '" + column.name() + 
                                             "' expects STRING, got " + value.type_name());
                }
            }
            if (column.indexed()) {
                ...
                bool dublicate_found = std::visit([&](auto& tree) -> bool {
                ...
                if (dublicate_found) {
                    throw std::runtime_error("Duplicate value for indexed column '" + 
