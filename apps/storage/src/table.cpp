...
Table::FastStr::FastStr(std::string_view value, int32_t record_id) : _record_id(record_id) {
    std::memset(_prefix, 0, PREFIX_SIZE);
    std::memcpy(_prefix, value.data(), PREFIX_SIZE <= value.size() ? PREFIX_SIZE : value.size());
}

bool Table::FastStr::operator<(FastStr other) const {
...
bool Table::FastStr::operator==(const FastStr& other) const {
    return _record_id == other._record_id && std::memcmp(_prefix, other._prefix, PREFIX_SIZE) == 0;
}

int Table::FastStr::SearchCmp(FastStr other) const { return std::memcmp(_prefix, other._prefix, PREFIX_SIZE); }

std::ostream& operator<<(std::ostream& os, const Table::FastStr& fast_str) {
    for (int i = 0; i < Table::FastStr::PREFIX_SIZE && fast_str._prefix[i]; ++i) {
...
std::optional<Record> Table::read_record(RecordAddress record_address, void* mem_ptr) {
    if (DEBUG) {
...
void Table::update_some_columns(Record& record, std::vector<Value> values, const std::vector<std::string>& column_names)
    const {
    if (DEBUG) {
...
void Table::validate_record(const Record& record) {
    ...
    for (size_t i = 0; i < _schema.size(); ++i) {
        ...
        if (column.indexed()) {
            ...
            if (dublicate_found) {
                throw std::
                    runtime_error("Duplicate value for indexed column '" + column.name() + "': " + value.to_string());
            }
        }
    }
}
...
void Table::print() { std::cout << *this; }

BStarPlusTree<uint32_t, RecordAddress>* Table::get_id_to_addr() { return &_id_to_addr; }
