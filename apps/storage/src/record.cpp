...
Record::Record(std::uint32_t id, std::uint32_t values_count)
        : _id(id), _fields(std::vector<Value>(values_count)) {
    if (values_count == 0) {
        throw std::runtime_error("[ERROR in qdb::storage::Record]: Record cannot have 0 columns!");
    }
}

Record::Record(std::uint32_t id, std::vector<Value> values)
        : _id(id), _fields(std::move(values)){};
...
auto Record::FromBinary(const std::uint8_t* data, std::uint32_t size, std::uint32_t record_id,
...
auto Record::Id() const -> std::uint32_t {
    return _id;
}

auto Record::Size() const -> std::uint32_t {
    return static_cast<std::uint32_t>(_fields.size());
}
...
auto Record::HasAddress() const -> bool {
...
auto Record::operator[](const std::size_t &index) -> Value& {
    return _fields[index];
}

auto Record::operator[](const std::size_t &index) const -> const Value& {
    return _fields[index];
}
