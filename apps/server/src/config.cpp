...
auto Config::
    New(std::string host,
        ...
        bool auto_start_storage,
        std::string storage_binary,
        std::string storage_root) -> Config {
    return Config(
        std::move(host),
        port,
        auth_required,
        std::move(jwt_secret),
        std::move(account_path),
        std::move(rbac_path),
        auto_start_storage,
        std::move(storage_binary),
...
auto Config::FromArguments(int argc, char** argv) -> std::optional<Config> {
    ...
    args::Flag
        no_storage(parser, "no-storage-autostart", "Do not launch qdb-storage processes", {"no-storage-autostart"});
    ...
    return Config::
        New(host.Get(),
            static_cast<std::uint32_t>(port.Get()),
            !no_auth.Get(),
            secret.Get(),
            accounts.Get(),
            rbac.Get(),
...
auto Config::AuthRequired() const -> bool { return auth_required_; }

auto Config::JwtSecret() const -> const std::string& { return jwt_secret_; }

auto Config::AccountPath() const -> const std::string& { return account_path_; }

auto Config::RbacPath() const -> const std::string& { return rbac_path_; }

auto Config::AutoStartStorage() const -> bool { return auto_start_storage_; }

auto Config::StorageBinary() const -> const std::string& { return storage_binary_; }

auto Config::StorageRoot() const -> const std::string& { return storage_root_; }
